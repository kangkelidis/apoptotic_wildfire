...
globals [; GLOBALS
        ...
        ; 2.11.1
         ...
         SAR_EFFORT_SAMPLES
         PARTICIPATION_DEBUG_TICK_TRACE
...
sar-robots-own [
  ...
  recent-staff-unavailable
  window-success-count
  window-failed-request-count
  window-staff-unavailable-count
  participation-pending-flag
  pending-unresolved-nearby
  pending-nearby-sars
  pending-active-value
  ignored-victim
  ignored-victim-ticks
]
...
to setup
  ...
  set DUPLICATE_ROBOT_CONTACTS               0
  set BUSY_STAFF_DISTRACTIONS                0
  set INTERFERENCE_DELAY_TOTAL               0
  set INTERFERENCE_DELAY_APPLICATIONS        0
  set DUPLICATE_DELAY_APPLICATIONS           0
  set BUSY_STAFF_DELAY_APPLICATIONS          0
  set ACTIVE_ROBOT_TICKS                     0
  set RESERVE_ROBOT_TICKS                    0
  set TASK_COMMITTED_ROBOT_TICKS             0
  set SAR_EFFORT_SAMPLES                     0
  set PARTICIPATION_DEBUG_TICK_TRACE         []
  ...
  ask agents [
    ...
    set support-request-owner nobody
  ]
...
to go
 ...
 ask sar-robots [
   decay-participation-signals
   update-task-commitment
   maybe-update-participation
 ]

 evaluate-pending-participation-decisions
 accumulate-sar-effort

 ask sar-robots [
   update-ignored-victim
   if victim-found != nobody and passenger-recovered? victim-found [
     release-victim-claim TRUE
     prepare-new-search
   ]
 ...
 if PARTICIPATION_DEBUG_ENABLED [
   record-participation-debug-state
 ]
 check-exit
...
to-report duplicate-robot-contacts
  report DUPLICATE_ROBOT_CONTACTS
end

to-report busy-staff-distractions
  report BUSY_STAFF_DISTRACTIONS
end

to-report interference-delay-total
  report INTERFERENCE_DELAY_TOTAL
end

to-report interference-delay-applications
  report INTERFERENCE_DELAY_APPLICATIONS
end

to-report duplicate-delay-applications
  report DUPLICATE_DELAY_APPLICATIONS
end

to-report busy-staff-delay-applications
  report BUSY_STAFF_DELAY_APPLICATIONS
end

to-report active-robot-ticks
  report ACTIVE_ROBOT_TICKS
end

to-report sar-effort-samples
  report SAR_EFFORT_SAMPLES
end

to-report reserve-robot-ticks
  report RESERVE_ROBOT_TICKS
end

to-report task-committed-robot-ticks
  report TASK_COMMITTED_ROBOT_TICKS
end

to-report participation-debug-tick-trace-csv
  if not PARTICIPATION_DEBUG_ENABLED [
    report ""
  ]
  ...
  report csv:to-string (fput header-row PARTICIPATION_DEBUG_TICK_TRACE)
end
...
to-report evacuation-finished?
  ...
  report count turtles with [breed != sar-robots and color != DEAD_PASSENGERS_COLOR] =  0
...
;  if count patches in-radius PROTOCOL_DISTANCE with [pcolor = blue] <= 0 [move-to back_step]
  ...
  if count agents with [color = PASSENGERS_COLOR] = 0 [
    ...
    if target-patch != nobody and test != nobody and target-patch != patch-here [
...
to decay-participation-signals
  set recent-success-rate recent-success-rate * 0.9
  set recent-failed-requests recent-failed-requests * 0.9
  set recent-staff-unavailable recent-staff-unavailable * 0.9
end

to update-task-commitment
  set task-committed-flag FALSE
  if victim-found != nobody [
    set task-committed-flag TRUE
  ]
...
to maybe-update-participation
  if ROBOT_PARTICIPATION_STRATEGY = "always" [
    set active-flag TRUE
    set reserve-flag FALSE
    set participating? TRUE
    stop
  ]

  if task-committed-flag [
    stop
  ]

  set decision-timer decision-timer - 1
  if decision-timer > 0 [
    stop
  ]

  set pending-unresolved-nearby get-unresolved-nearby
  set pending-nearby-sars get-nearby-sars
  ifelse active-flag [
    set pending-active-value 1
  ][
    set pending-active-value 0
  ]
  set participation-pending-flag TRUE
end

to evaluate-pending-participation-decisions
  let pending-robots sort (sar-robots with [participation-pending-flag])
  if empty? pending-robots [
    stop
  ]

  let observations-json ""
  let first-item TRUE
  foreach pending-robots [
    if not first-item [
      set observations-json word observations-json ","
    ]
    set observations-json word observations-json [build-participation-observation-json] of ?
    set first-item FALSE
  ]

  let curl-command (word "curl -s -X POST http://localhost:5000/robot_participation_batch -H \"Content-Type: application/json\" -d '{"
  ...
  if member? "ERROR" controller-response [
    error (word "Problems invoking participation controller: " controller-response)
  ]

  let controller-output read-from-string controller-response
  if not is-list? controller-output [
    error (word "Invalid participation controller response: " controller-response)
  ]
  if length controller-output != length pending-robots [
    error (word "Participation batch response size mismatch: " controller-response)
  ]

  foreach controller-output [
    let controller-row ?
    if not is-list? controller-row [
      error (word "Invalid participation batch row: " controller-row)
    ]
    let robot-id item 0 controller-row
    let matching-robots sar-robots with [who = robot-id]
    if not any? matching-robots [
      error (word "No SAR robot found for participation row " controller-row)
    ]
    ask matching-robots [
      apply-participation-decision controller-row
    ]
  ]

 ask sar-robots with [participation-pending-flag] [
    ifelse PARTICIPATION_DECISION_INTERVAL > 0 [
      set decision-timer PARTICIPATION_DECISION_INTERVAL
    ][
      set decision-timer 0
    ]
    reset-participation-window-counters
    set participation-pending-flag FALSE
  ]
end

to-report build-participation-observation-json
  let nearest-victim-distance get-nearest-victim-distance
  let available-bystanders get-available-bystanders
  let available-staff get-available-staff
  let task-committed-value 0
  let reserve-value 0
  if task-committed-flag [ set task-committed-value 1 ]
  if reserve-flag [ set reserve-value 1 ]

  report (word "{"
               "\"simulation_id\":\"" SIMULATION_ID "\","
               "\"robot_id\":" who ","
               "\"robot_rank\":" robot-rank ","
               "\"tick\":" ticks ","
               "\"unresolved_nearby\":" pending-unresolved-nearby ","
               "\"nearest_victim_distance\":" nearest-victim-distance ","
               "\"available_bystanders\":" available-bystanders ","
               "\"available_staff\":" available-staff ","
               "\"recent_success_rate\":" recent-success-rate ","
               "\"nearby_sars\":" pending-nearby-sars ","
               "\"recent_failed_requests\":" recent-failed-requests ","
               "\"recent_staff_unavailable\":" recent-staff-unavailable ","
               "\"window_success_count\":" window-success-count ","
               "\"window_failed_request_count\":" window-failed-request-count ","
               "\"window_staff_unavailable_count\":" window-staff-unavailable-count ","
               "\"task_committed\":" task-committed-value ","
               "\"reserve_flag\":" reserve-value ","
               "\"active_flag\":" pending-active-value ","
               "\"distance_to_nearest_victim\":" nearest-victim-distance
               "}")
end

to apply-participation-decision [controller-output]
  let action item 1 controller-output
  set last-score item 2 controller-output
  set tau-local item 3 controller-output
  set regret-ema-local item 4 controller-output

  if action = "return-to-reserve" and active-flag [
    set active-flag FALSE
    set reserve-flag TRUE
...
to reset-participation-window-counters
  set window-success-count 0
  set window-failed-request-count 0
  set window-staff-unavailable-count 0
end

to record-participation-debug-state
  let active-count count sar-robots with [active-flag or task-committed-flag]
  let reserve-count count sar-robots with [reserve-flag]
  let task-committed-count count sar-robots with [task-committed-flag]
  let unresolved-count count agents with [
    st_fall = 1 and color != DEAD_PASSENGERS_COLOR
  ]
  let attended-count count agents with [
    st_fall = 1 and help-in-progress = TRUE and color != DEAD_PASSENGERS_COLOR
  ]
...
to accumulate-sar-effort
  set SAR_EFFORT_SAMPLES (SAR_EFFORT_SAMPLES + 1)
  set ACTIVE_ROBOT_TICKS (
    ACTIVE_ROBOT_TICKS + count sar-robots with [active-flag or task-committed-flag]
  )
  set RESERVE_ROBOT_TICKS (
    RESERVE_ROBOT_TICKS + count sar-robots with [reserve-flag]
  )
  set TASK_COMMITTED_ROBOT_TICKS (
    TASK_COMMITTED_ROBOT_TICKS + count sar-robots with [task-committed-flag]
  )
end

to-report get-sar-heading-delta
  let raw ((who * 73 + ticks * 37 + robot-rank * 19) mod 90)
  report 45 - raw
end

to-report get-unresolved-nearby
  report count agents in-radius SAR_ROBOT_OBSERVATION_DISTANCE with [
    st_fall = 1 and color != DEAD_PASSENGERS_COLOR
  ]
end

to-report get-nearest-victim-distance
  let unresolved agents in-radius SAR_ROBOT_OBSERVATION_DISTANCE with [
    st_fall = 1 and color != DEAD_PASSENGERS_COLOR
  ]
  ifelse any? unresolved [
    report [distance myself] of min-one-of unresolved [distance myself]
  ][
    report -1
  ]
end

to-report get-available-bystanders
  report count agents in-radius SAR_ROBOT_OBSERVATION_DISTANCE with [
    st_fall = 0 and st_dead = 0 and st_leader = 1 and agent_to_help = nobody
  ]
end

to-report get-available-staff
  report count staff in-radius SAR_ROBOT_OBSERVATION_DISTANCE with [
    assistance-required = nobody
  ]
end

to-report get-nearby-sars
  report count other sar-robots in-radius SAR_ROBOT_OBSERVATION_DISTANCE with [active-flag]
end

to update-ignored-victim
  if ignored-victim = nobody [
    stop
  ]

  if not is-turtle? ignored-victim [
    set ignored-victim nobody
    set ignored-victim-ticks 0
    stop
  ]

  if [st_fall] of ignored-victim = 0 [
    set ignored-victim nobody
    set ignored-victim-ticks 0
    stop
  ]

  if [help-in-progress] of ignored-victim = TRUE [
    stop
  ]

  if ignored-victim-ticks > 0 [
    set ignored-victim-ticks (ignored-victim-ticks - 1)
  ]
...
to mark-victim-ignored [fallen-passenger]
  if fallen-passenger = nobody [
    stop
  ]

  set ignored-victim fallen-passenger
  set ignored-victim-ticks OBSERVATION_DISTANCE
end

to release-victim-claim [release-help-flag?]
  if victim-found = nobody [
    stop
  ]

  let target-victim victim-found
  if [support-request-owner] of target-victim = self [
    ask target-victim [
      set support-request-owner nobody
      if release-help-flag? and st_fall = 1 [
        set help-in-progress FALSE
      ]
    ]
  ]
...
to apply-interference-delay [fallen-passenger interference-source]
  if fallen-passenger = nobody [
    stop
  ]

  if [st_fall] of fallen-passenger = 1 [
    let previous-fall-length [fall-length] of fallen-passenger
    let new-fall-length min (list DEFAULT_FALL_LENGTH (previous-fall-length + ROBOT_INTERFERENCE_DELAY))
    let applied-delay max (list 0 (new-fall-length - previous-fall-length))
    if applied-delay > 0 [
      ask fallen-passenger [
        set fall-length new-fall-length
      ]
      set INTERFERENCE_DELAY_APPLICATIONS (INTERFERENCE_DELAY_APPLICATIONS + 1)
      if interference-source = "duplicate" [
        set DUPLICATE_DELAY_APPLICATIONS (DUPLICATE_DELAY_APPLICATIONS + 1)
      ]
      if interference-source = "busy-staff" [
        set BUSY_STAFF_DELAY_APPLICATIONS (BUSY_STAFF_DELAY_APPLICATIONS + 1)
      ]
      set INTERFERENCE_DELAY_TOTAL (INTERFERENCE_DELAY_TOTAL + applied-delay)
    ]
  ]
end
...
to-report get-nearest-busy-staff-member
  ; For the SAR robot, to get the closest busy staff member.
  let result nobody
  let busy-staff staff with [assistance-required != nobody]

  if any? busy-staff [
    let nearest-staff-member min-one-of busy-staff [
      distance myself
    ]
...
to request-staff-support
  ...
  let nearest-busy-staff-member get-nearest-busy-staff-member
  ...
  ifelse nearest-staff-member != nobody [
    ...
    set recent-success-rate min (list 1 (recent-success-rate + 0.5))
    set window-success-count window-success-count + 1
    prepare-new-search
  ][
    ; No staff available for help. Distract a busy staff member instead.
    log-turtle "No staff available. Victim waiting: " target-victim
    if nearest-busy-staff-member != nobody [
      let distracted-victim [assistance-required] of nearest-busy-staff-member
      if not passenger-recovered? distracted-victim [
        apply-interference-delay distracted-victim "busy-staff"
        set BUSY_STAFF_DISTRACTIONS (BUSY_STAFF_DISTRACTIONS + 1)
      ]
    ]
    set recent-staff-unavailable min (list 1 (recent-staff-unavailable + 0.5))
    set window-staff-unavailable-count window-staff-unavailable-count + 1
    release-victim-claim TRUE
    prepare-new-search
  ]
...
to prepare-new-search
  ...
  if victim-found != nobody [
    release-victim-claim FALSE
    mark-victim-ignored victim-found
  ]
...
to request-passanger-help
  ...
  ; The robot coordinates support only. It does not amplify recovery speed.
  ...
  ifelse do-help [
    ...
    ; Helper refused. Leave the victim unattended and move on.
    log-turtle "Help rejected. Bystander:" candidate-helper
    set recent-failed-requests min (list 1 (recent-failed-requests + 0.5))
    set window-failed-request-count window-failed-request-count + 1
    release-victim-claim TRUE
    prepare-new-search
  ]
end
...
to move-sar-robots
  ...
  ; Use a deterministic patrol delta so SAR motion does not perturb the main RNG stream.
  set heading (heading + get-sar-heading-delta)
  jump 1
...
to place-sar-robots
  ...
  create-sar-robots NUM_OF_ROBOTS [
    ...
    ifelse ROBOT_PARTICIPATION_STRATEGY = "always" [
      set participating? TRUE
      set active-flag TRUE
      set reserve-flag FALSE
    ][
    ...
    ifelse PARTICIPATION_DECISION_INTERVAL > 0 [
      set decision-timer random PARTICIPATION_DECISION_INTERVAL
    ][
      set decision-timer 0
    ]
    set task-committed-flag FALSE
    set last-score 0
    set tau-local 0
    set regret-ema-local 0
    set recent-success-rate 0
    set recent-failed-requests 0
    set recent-staff-unavailable 0
    set window-success-count 0
    set window-failed-request-count 0
    set window-staff-unavailable-count 0
    set participation-pending-flag FALSE
    set pending-unresolved-nearby 0
    set pending-nearby-sars 0
    set pending-active-value 0
    set ignored-victim nobody
    set ignored-victim-ticks 0
    set support-strategy get-support-strategy
...
to request-support
  ...
  if [help-in-progress] of victim-found and [support-request-owner] of victim-found != self [
    apply-interference-delay victim-found "duplicate"
    set DUPLICATE_ROBOT_CONTACTS (DUPLICATE_ROBOT_CONTACTS + 1)
    set recent-failed-requests min (list 1 (recent-failed-requests + 0.5))
    set window-failed-request-count window-failed-request-count + 1
    prepare-new-search
    stop
...
to search-fallen-passengers
  ...
  let list-fallen-passengers agents in-radius SAR_ROBOT_OBSERVATION_DISTANCE with [
    st_fall = 1 and
    color != DEAD_PASSENGERS_COLOR and
    ([ignored-victim] of myself = nobody or self != [ignored-victim] of myself or [ignored-victim-ticks] of myself <= 0)
  ]

  ifelse count list-fallen-passengers > 0 [
    let unattended-passengers list-fallen-passengers with [help-in-progress = FALSE]
    let passenger-to-help nobody
    ifelse any? unattended-passengers [
      set passenger-to-help one-of unattended-passengers
    ][
      set passenger-to-help one-of list-fallen-passengers
    ]
    set victim-found passenger-to-help
    
    if [help-in-progress] of passenger-to-help = FALSE [
      ask passenger-to-help [
        set help-in-progress TRUE
        set support-request-owner myself
      ]
    ]
  ][
    prepare-new-search
