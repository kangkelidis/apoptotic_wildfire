...
def load_config(config_file_path: str) -> dict[str, Any]:
    ...
    cached_config = CONFIG_CACHE.get(config_file_path)
    ...
    CONFIG_CACHE[config_file_path] = config
    logger.debug('Config checked and loaded.')
    return config


def get_max_time(config_file_path: str = CONFIG_FILE) -> int:
    """
...
def _check_for_range(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in parameters.items():
        if isinstance(value, dict) and 'start' in value and 'end' in value:
            # np.arange can introduce floating-point drift (e.g. 0.450000000002) in the grid.
