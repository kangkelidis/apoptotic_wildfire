"""
This module defines the classes and functions necessary for executing the NetLogo model simulations.

Classes:
- Updatable: A base class that provides a method to update object attributes.
- NetLogoParams: Stores the parameters required to configure and run a simulation.
- Result: Holds the results of a single simulation, including metrics such as evacuation ticks.
- Scenario: Represents a simulation scenario, encapsulating the parameters, simulations,
            and results associated with it.
- Simulation: Encapsulates the details necessary to run a single simulation within a scenario.
"""
from __future__ import annotations

import copy
import random
from typing import Any, Optional, Union

import pandas as pd  # type: ignore
from src.adaptation_strategy import AdaptationStrategy
from src.participation_controller import ParticipationController
from utils.helper import convert_camelCase_to_snake_case, setup_logger

DEFAULT_ADAPTATION_STRATEGY = "RandomStrategy"
DEFAULT_ROBOT_PARTICIPATION_STRATEGY = "always"
SUPPORTED_ROBOT_PARTICIPATION_STRATEGIES = {"always", "frustration", "labella"}


class Updatable(object):
    logger = setup_logger()

    def update(self, params: dict) -> None:
        """
        Updates the object's parameters that are in the provided dictionary.

        Args:
            params: A dictionary containing the parameters to update.
        """
        for key, value in params.items():
            attr_name = convert_camelCase_to_snake_case(key)
            if hasattr(self, attr_name):
                setattr(self, attr_name, value)


class NetLogoParams(Updatable):
    """
    Holds the NetLogo parameters for a simulation.

    Attributes:
    - seed: The seed to create the seed for the Netlogo simulation.
    - netlogo_seed: The seed for the NetLogo simulation.
    - num_of_samples: The number of simulations to run.
    - num_of_robots: The number of robots in the simulation.
    - num_of_passengers: The number of passengers in the simulation.
    - num_of_staff: The number of staff in the simulation.
    - fall_length: The number of ticks of fall lasts in the simulation.
    - fall_chance: The chance of a fall happening in the simulation.
    - robot_persuasion_factor: A multiplier to the helping chance in the Netlogo simulation.
    - max_netlogo_ticks: The maximum number of NetLogo ticks to run the simulation.
    - room_type: The type of room in the simulation.
    - enable_video: Whether to enable video recording of the simulation.
    """
    def __init__(self):
        self.seed = 0
        self.netlogo_seed = None
        self.num_of_samples = 30
        self.num_of_robots = 1
        self.num_of_passengers = 800
        self.num_of_staff = 10
        self.fall_length = 500
        self.fall_chance = 0.05
        self.robot_persuasion_factor = 1
        self.robot_participation_strategy = DEFAULT_ROBOT_PARTICIPATION_STRATEGY
        self.robot_participation_config: dict = {}
        self.participation_decision_interval = 20
        self.participation_debug_enabled = False
        self.robot_interference_delay = 0
        self.max_netlogo_ticks = 2000
        self.room_type = 8
        self.enable_video = False

    def duplicate(self):
        new_obj = NetLogoParams()
        new_obj.update(copy.deepcopy(self.__dict__))
        return new_obj


class Result(Updatable):
    """
    Holds the results of a simulation.

    Attributes:
    - seed: The seed used for the simulation.
    - evacuation_ticks: The number of ticks it took to evacuate the room.
    - evacuation_time: The time it took to execute the simulation.
    - robot_actions: A list of the actions performed by the robots.
    - robot_responses: A list of the responses from the zero responder.
    - robot_contacts: The number of fallen victims the robot made contact with.
    - success: Whether the simulation was successful (finished on time and no errors).
    """
    def __init__(self,
                 netlogo_seed: int = 0,
                 evacuation_ticks: Optional[int] = None,
                 evacuation_time: Optional[float] = None,
                 success: bool = False,
                 failure_reason: Optional[str] = None,
                 attempt_count: int = 0,
                 duplicate_robot_contacts: int = 0,
                 busy_staff_distractions: int = 0,
                 interference_delay_total: float = 0,
                 interference_delay_applications: int = 0,
                 duplicate_delay_applications: int = 0,
                 busy_staff_delay_applications: int = 0,
                 active_robot_ticks: int = 0,
                 reserve_robot_ticks: int = 0,
                 task_committed_robot_ticks: int = 0,
                 runtime_robot_participation_strategy: Optional[str] = None,
                 runtime_robot_interference_delay: float = 0,
                 participation_decision_count: int = 0,
                 participation_stay_active_decisions: int = 0,
                 participation_return_to_reserve_decisions: int = 0,
                 participation_reactivation_decisions: int = 0,
                 participation_reserve_wait_decisions: int = 0,
                 mean_active_robots: float = 0,
                 peak_active_robots: int = 0,
                 mean_reserve_robots: float = 0,
                 peak_reserve_robots: int = 0,
                 mean_task_committed_robots: float = 0,
                 mean_unresolved_fallen: float = 0,
                 mean_attended_fallen: float = 0,
                 reserve_with_unresolved_ticks: int = 0,
                 debug_participation_trace: Optional[list[dict[str, Any]]] = None,
                 debug_tick_trace_csv: str = "",
                 ) -> None:
        self.evacuation_ticks = evacuation_ticks
        self.evacuation_time = evacuation_time
        self.failure_reason = failure_reason
        self.attempt_count = attempt_count
        self.robot_actions: list[str] = []
        self.robot_responses: list[str] = []
        self.robot_contacts: int = 0
        self.duplicate_robot_contacts = duplicate_robot_contacts
        self.busy_staff_distractions = busy_staff_distractions
        self.interference_delay_total = interference_delay_total
        self.interference_delay_applications = interference_delay_applications
        self.duplicate_delay_applications = duplicate_delay_applications
        self.busy_staff_delay_applications = busy_staff_delay_applications
        self.active_robot_ticks = active_robot_ticks
        self.reserve_robot_ticks = reserve_robot_ticks
        self.task_committed_robot_ticks = task_committed_robot_ticks
        self.runtime_robot_participation_strategy = runtime_robot_participation_strategy
        self.runtime_robot_interference_delay = runtime_robot_interference_delay
        self.participation_decision_count = participation_decision_count
        self.participation_stay_active_decisions = participation_stay_active_decisions
        self.participation_return_to_reserve_decisions = participation_return_to_reserve_decisions
        self.participation_reactivation_decisions = participation_reactivation_decisions
        self.participation_reserve_wait_decisions = participation_reserve_wait_decisions
        self.mean_active_robots = mean_active_robots
        self.peak_active_robots = peak_active_robots
        self.mean_reserve_robots = mean_reserve_robots
        self.peak_reserve_robots = peak_reserve_robots
        self.mean_task_committed_robots = mean_task_committed_robots
        self.mean_unresolved_fallen = mean_unresolved_fallen
        self.mean_attended_fallen = mean_attended_fallen
        self.reserve_with_unresolved_ticks = reserve_with_unresolved_ticks
        self.debug_participation_trace = debug_participation_trace or []
        self.debug_tick_trace_csv = debug_tick_trace_csv
        self.success = success
        self.netlogo_seed = netlogo_seed

    def __str__(self):
        return (f"Evacuation ticks: {self.evacuation_ticks}, "
                f"Evacuation time: {self.evacuation_time:.2f}, "
                f"Seed: {self.netlogo_seed}")


class Scenario(Updatable):
    """
    Represents a simulation scenario. Contains the parameters and results for the scenario
    and a list of its simulation objects.

    Attributes:
    - name: The name of the scenario.
    - description: A description of the scenario.
    - netlogo_params: The NetLogo parameters for the scenario.
    - adaptation_strategy: The adaptation strategy to use in the scenario.
    - simulations: A list of Simulation objects for the scenario.
    - results: A list of Result objects for the scenario.
    - logger: A logger object for logging messages.
    - simulation_indices_with_video: A set of indices of simulations with video enabled.
    - simulation_ids_with_video: A list of ids that have video enabled.
    """

    @staticmethod
    def find_by_name(scenario_name: str, scenarios: list['Scenario']) -> 'Scenario':
        """
        Returns the scenario with the given name from the list of scenarios.

        Args:
            scenario_name: The name of the scenario to find.
            scenarios: The list of scenarios.

        Returns:
            The scenario object.
        """
        for scenario in scenarios:
            if scenario.name == scenario_name:
                return scenario
        raise NameError(f"No matching scenario found for name {scenario_name} in {scenarios}")

    def __init__(self) -> None:
        self.name = ''
        self.description = ''
        self.netlogo_params = NetLogoParams()
        self.adaptation_strategy: Optional[AdaptationStrategy] = None
        self.participation_controller: Optional[ParticipationController] = None
        self.participation_params: dict = {}
        self.simulations: list[Simulation] = []
        self.results: list[Result] = []
        self.logger = setup_logger()
        self.simulation_indices_with_video: set[int] = set()
        self.simulation_ids_with_video: list[str] = []

    def _get_num_robots_scalar(self) -> Optional[int]:
        value = self.netlogo_params.num_of_robots
        if isinstance(value, bool):
            return int(value)
        if isinstance(value, (int, float)):
            return int(value)
        return None

    def configure_controllers(self, strategy_name: Union[str, list, None] = None) -> None:
        num_of_robots = self._get_num_robots_scalar()

        if isinstance(strategy_name, str):
            self.adaptation_strategy = AdaptationStrategy.get_strategy(strategy_name, self)
        elif num_of_robots is not None and num_of_robots > 0:
            self.adaptation_strategy = AdaptationStrategy.get_strategy(
                DEFAULT_ADAPTATION_STRATEGY,
                self,
            )
        else:
            self.adaptation_strategy = None

        if num_of_robots is not None and num_of_robots > 0:
            self.participation_controller = ParticipationController.get_controller(
                self.netlogo_params.robot_participation_strategy,
                self,
            )
        else:
            self.participation_controller = None

    def update(self, params: dict) -> None:
        super().update(params)
        self.netlogo_params.update(params)
        strategy_name: Union[str, list, None] = params.get('adaptation_strategy', None)

        participation_strategy = str(
            self.netlogo_params.robot_participation_strategy
            or DEFAULT_ROBOT_PARTICIPATION_STRATEGY
        ).lower()
        if participation_strategy not in SUPPORTED_ROBOT_PARTICIPATION_STRATEGIES:
            raise ValueError(
                "robotParticipationStrategy must be one of "
                f"{sorted(SUPPORTED_ROBOT_PARTICIPATION_STRATEGIES)}. "
                f"Got: {self.netlogo_params.robot_participation_strategy}"
            )
        self.netlogo_params.robot_participation_strategy = participation_strategy
        if self.netlogo_params.robot_participation_config is None:
            self.netlogo_params.robot_participation_config = {}
        if not isinstance(self.netlogo_params.robot_participation_config, dict):
            raise ValueError("robotParticipationConfig must be an object/dictionary if provided.")
        self.participation_params = copy.deepcopy(self.netlogo_params.robot_participation_config)
        if "decisionInterval" in self.participation_params:
            self.netlogo_params.participation_decision_interval = int(
                self.participation_params["decisionInterval"]
            )
        self.netlogo_params.participation_debug_enabled = bool(
            self.participation_params.get("debug", False)
        )

        self.configure_controllers(strategy_name)

    def duplicate(self) -> 'Scenario':
        new_scenario = Scenario()
        new_scenario.name = self.name
        new_scenario.description = self.description
        new_scenario.adaptation_strategy = self.adaptation_strategy
        new_scenario.participation_controller = self.participation_controller
        new_scenario.participation_params = copy.deepcopy(self.participation_params)
        new_scenario.simulations = self.simulations[:]
        new_scenario.results = self.results[:]
        new_scenario.netlogo_params.update(copy.deepcopy(self.netlogo_params.__dict__))
        # update the adaptation strategy scenario attribute to the new scenario
        if new_scenario.adaptation_strategy:
            new_scenario.adaptation_strategy.scenario = new_scenario
        if new_scenario._get_num_robots_scalar() is not None:
            strategy_name = None
            if self.adaptation_strategy is not None:
                strategy_name = str(self.adaptation_strategy)
            new_scenario.configure_controllers(strategy_name)
        return new_scenario

    def _check_video(self, simulation: Simulation) -> None:
        """
        Checks if video is enabled and sets the NetLogoParams in the simulation accordingly.

        1. If false or not set, it sets the enable_video attribute to false.
        2. If it is 'all' or true or a number bigger than length of simulations,
             all simulations will have video enabled.
        3. If it is a list of simulation indices, only those simulations will have video enabled.
        4. If it is a number n, n random simulations will have video enabled,
             If n is greater than len simulations, all simulations will have video enabled.

        Args:
            simulation: The simulation object to check.
        """
        if simulation.netlogo_params.enable_video:
            # case 2: enable_video is True or 'all'
            if simulation.netlogo_params.enable_video is True or \
                    simulation.netlogo_params.enable_video == 'all':
                simulation.netlogo_params.enable_video = True
                self.simulation_ids_with_video.append(simulation.id)

            # case 3: enable_video is a list of simulation indices
            elif isinstance(simulation.netlogo_params.enable_video, list):
                if simulation.index in simulation.netlogo_params.enable_video:
                    simulation.netlogo_params.enable_video = True
                    self.simulation_ids_with_video.append(simulation.id)
                else:
                    simulation.netlogo_params.enable_video = False

            # case 4: enable_video is a number
            elif isinstance(simulation.netlogo_params.enable_video, int):
                if simulation.netlogo_params.enable_video > self.netlogo_params.num_of_samples:
                    simulation.netlogo_params.enable_video = True
                    self.simulation_ids_with_video.append(simulation.id)
                else:
                    # run the first time only
                    if len(self.simulation_indices_with_video) == 0:
                        # randomly select n simulations to have video enabled
                        self.simulation_indices_with_video = \
                            set(random.sample(range(0, self.netlogo_params.num_of_samples),
                                              simulation.netlogo_params.enable_video))
                    if simulation.index in self.simulation_indices_with_video:
                        simulation.netlogo_params.enable_video = True
                        self.simulation_ids_with_video.append(simulation.id)

            #  case 1: enable_video is False or invalid
            else:
                simulation.netlogo_params.enable_video = False

    def build_simulations(self) -> None:
        """
        Builds the simulation objects for this scenario and saves them in a list.
        """
        for simulation_index in range(self.netlogo_params.num_of_samples):
            simulation = Simulation(self.name, simulation_index, self.netlogo_params)
            self._check_video(simulation)
            self.simulations.append(simulation)
        self.logger.debug(f"Finished building simulations for scenario: {self.name}. "
                          f"Size of list: {len(self.simulations)}")

    def get_data(self) -> pd.DataFrame:
        """
        Returns a DataFrame containing the scenario data and results of its simulations.

        Returns:
            A DataFrame containing the scenario data.
        """
        params = copy.deepcopy(self.netlogo_params.__dict__)
        # rename for clarity
        params['param_seed'] = params['seed']
        del params['seed']
        info = {'scenario': self.name, 'strategy': self.adaptation_strategy}
        if self.participation_controller is not None:
            info['participation_controller'] = self.participation_controller
        for key, value in self.participation_params.items():
            info[f'participation_param_{key}'] = value

        scenario_data = []
        for simulation in self.simulations:
            result = {
                key: value
                for key, value in simulation.result.__dict__.items()
                if not key.startswith("debug_")
            }

            scenario_data.append({"simulation_id": simulation.id, **info, **params, **result})

        return pd.DataFrame(scenario_data)


class Simulation(Updatable):
    """
    A simulation object with the necessary parameters to run a simulation in NetLogo.
    Also contains the results object created by the simulation.

    Attributes:
    - scenario_name: The name of the scenario.
    - index: The index of the simulation.
    - id: The unique ID of the simulation.
    - netlogo_params: The NetLogo parameters for the simulation.
    - result: The Result object for the simulation.
    - seed: The seed used for the simulation.
    - netlogo_seed: The seed used in NetLogo for the simulation.
    """

    @staticmethod
    def get_scenario_name(simulation_id: str) -> str:
        """
        Extracts the scenario name from a simulation ID.

        This function takes a simulation ID as input and returns the scenario name
        by splitting the ID on the "_" character and returning the first part.

        Args:
            simulation_id: The simulation ID.

        Returns:
            The scenario name.
        """
        if "_" not in simulation_id:
            raise ValueError(f"simulation_id must contain an underscore ('_'). {simulation_id}")
        return simulation_id.split("_")[0]

    @staticmethod
    def get_index(simulation_id: str) -> str:
        """
        Extracts the scenario index from a simulation ID.

        This function takes a simulation ID as input and returns the scenario index
        by splitting the ID on the "_" character and returning the second part.

        Args:
            simulation_id: The simulation ID.

        Returns:
            The scenario index.
        """
        if "_" not in simulation_id:
            raise ValueError(f"simulation_id must contain an underscore ('_'). {simulation_id}")
        return simulation_id.split("_")[1]

    @staticmethod
    def generate_id(scenario_name: str, index: int) -> str:
        """
        Generates a simulation ID based on the scenario and index.

        This function takes a scenario name and an index as input, and returns a
        simulation ID in the format "scenario_index".

        Args:
            scenario_name: The name of the scenario.
            index: The index of the simulation.

        Returns:
            The generated simulation ID.
        """
        scenario_name = scenario_name.replace("_", "-")
        return scenario_name + "_" + str(index)

    @staticmethod
    def find_by_id(scenario_s: Union[list[Scenario], Scenario],
                   simulation_id: str) -> Simulation:
        """
        Returns the simulation with the given ID from the list of scenarios.

        Args:
            scenario_s: Either a list of scenarios or a single scenario.
            simulation_id: The ID of the simulation to find.

        Returns:
            The Simulation object.
        """
        if isinstance(scenario_s, list):
            scenario_name = Simulation.get_scenario_name(simulation_id)
            scenario_s = Scenario.find_by_name(scenario_name, scenario_s)

        simulation: Optional[Simulation] = \
            next((s for s in scenario_s.simulations if s.id == simulation_id), None)

        if simulation is None:
            raise NameError(f"No matching simulation found for ID {simulation_id}")

        return simulation

    def __init__(self, scenario_name: str, index: int, netlogo_params: NetLogoParams) -> None:
        self.scenario_name = scenario_name
        self.index = index
        self.id = Simulation.generate_id(scenario_name, index)
        self.netlogo_params = netlogo_params.duplicate()
        self.result: Result = Result()
        self.seed = self.generate_seed(index)
        # Optional explicit NetLogo seed; defaults to the seed in netlogo_params.
        self.netlogo_seed = None

    def generate_seed(self, index) -> int:
        """
        Generates a seed for the simulation based on the netlogo_params seed and index.

        Each simulation round will have the same seed, unless the netlogo_params seed is 0.
        Simulations in batch runs will behave the way.
        The seed number must be an integer in the range -2147483648 to 2147483647.

        Args:
            index: The index of the simulation.

        Returns:
            seed: An integer seed for the simulation.
        """
        #  use the netlogo random-seed if 0
        if self.netlogo_params.seed == 0:
            return 0
        # Specific seed provided in config
        if self.netlogo_params.netlogo_seed is not None:
            return self.netlogo_params.netlogo_seed

        # Generate a seed based on the netlogo_params seed and the index
        random.seed(self.netlogo_params.seed * (index + 1))
        while True:
            seed: int = random.randint(-2147483647, 2147483646)
            if seed != 0:
                break
        return seed

    def add_action(self, action: str) -> None:
        self.result.robot_actions.append(action)
        self._add_contact(action)

    def add_response(self, response: str) -> None:
        self.result.robot_responses.append(response)
        self._add_contact(response)

    def add_participation_decisions(
        self,
        rows: list[dict[str, Any]],
        keep_trace: bool = False,
    ) -> None:
        self.result.participation_decision_count += len(rows)
        for row in rows:
            action = row.get("action")
            if action == ParticipationController.ACTION_STAY_ACTIVE:
                self.result.participation_stay_active_decisions += 1
            if action == ParticipationController.ACTION_RETURN_TO_RESERVE:
                self.result.participation_return_to_reserve_decisions += 1
            if row.get("reactivated_from_reserve"):
                self.result.participation_reactivation_decisions += 1
            if row.get("stayed_in_reserve"):
                self.result.participation_reserve_wait_decisions += 1

        if keep_trace:
            self.result.debug_participation_trace.extend(rows)

    def _add_contact(self, action_or_response) -> None:
        """
        To count contacts with fallen victims.

        This method is called when the robot's interaction with the victim ends.
        Tha only happens if the robot calls for staff support or when the helper accepts to help.

        Args:
            action_or_response: The action or response from the robot.
        """
        accepted_responses = ["true", AdaptationStrategy.CALL_STAFF_ROBOT_ACTION]
        if action_or_response in accepted_responses:
            self.result.robot_contacts += 1
            self.logger.debug(f"Contact with fallen victim: {self.result.robot_contacts}")
